...
`supabase/schema.sql` を更新した場合は、既存プロジェクトでも同じSQLを再実行してください。`customer_name`, `payment_date`, `deposit_due_on`, `payment_completed`, `deposit_completed` などの追加列はこの再実行で反映されます。
